...
function StrengthMeter({ password }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {labels[score] && <p className="text-xs text-gray-400 font-medium">{labels[score]}</p>}
    </div>
  )
}
...
export default function ResetPassword() {
  ...
  const handleReset = async (data) => {
    setServerError('')
    try {
      await API.post('/auth/reset', { token, newPassword: data.password })
      setSubmitSuccess(true)
      setTimeout(() => navigate('/login'), 3000)
    } catch (err) {
      setServerError(err.response?.data?.message || 'Invalid or expired reset link. Please request a new one.')
    }
  }
      ...
      <div className="hidden lg:flex lg:w-[42%] bg-gray-900 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="z-10 space-y-6">
          ...
          <div className="flex flex-col gap-3">
            {['At least 8 characters long', 'Mix uppercase & lowercase', 'Include numbers or symbols'].map((tip) => (
          ...
          <div className="w-full max-w-[420px]">
            ...
            {submitSuccess ? (
              <div className="flex flex-col items-center py-10 gap-4 text-center">
                ...
                <Link to="/login"
                  className="mt-2 w-full flex items-center justify-center gap-2 py-3.5 bg-gray-900 text-white text-sm font-semibold rounded-xl hover:bg-gray-700 transition-all duration-200">
              ...
              <>
                <div className="mb-8">
                  <h1 className="font-display text-3xl font-bold text-gray-900 mb-2 tracking-tight">New password</h1>
                ...
                <form onSubmit={handleSubmit(handleReset)} noValidate className="space-y-5">
                  <div className="space-y-2">
                    <Field label="New Password" id="password" icon={Lock}
                  ...
                  <Field label="Confirm Password" id="confirmPassword" icon={Lock}
                    ...
                    register={register('confirmPassword')} error={errors.confirmPassword?.message}
                    rightSlot={
                  ...
                  {serverError && (
                    <div className="p-3 bg-red-50 border border-red-100 rounded-xl">
                      ...
                      <Link to="/forgot-password" className="text-xs text-red-400 hover:text-red-600 underline mt-1 block">
                  ...
                  <button type="submit" disabled={isSubmitting}
                    ...
                    {isSubmitting ? <><Loader2 size={16} className="animate-spin" /> Updating…</>
